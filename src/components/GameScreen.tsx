...
import { SongData, Difficulty, DrumPartId, RhythmNote, JudgmentType, ScoreState, JudgmentFeedback, PlayerSettings } from '../types';
...
export const GameScreen: React.FC<GameScreenProps> = ({
  ...
  const updateNotesAndGlows = useCallback(
    (time: number) => {
      const approachWindow = settings.approachSpeed || 0.45; // seconds before hit
      const missThreshold = 0.14; // 140ms after note time is a MISS
      ...
      notesRef.current.forEach((n) => {
        ...
        // Approaching note: show contracting ring
        if (timeDiff > 0 && timeDiff <= approachWindow) {
          const approach = 1 - timeDiff / approachWindow;
          if (approach > (updatedGlows[n.part]?.approachProgress || 0)) {
            updatedGlows[n.part] = {
        ...
        if (Math.abs(timeDiff) <= 0.08) {
          const intensity = 1 - Math.abs(timeDiff) / 0.08;
          updatedGlows[n.part] = {
            glowIntensity: Math.max(updatedGlows[n.part].glowIntensity, intensity),
            approachProgress: 1,
            noteId: n.id,
          };
        }
        ...
        if (timeDiff < -missThreshold) {
          ...
          n.judgment = 'MISS';
          hasMissed = true;
          triggerFeedback('MISS', n.part, undefined);
        }
      });
    ...
    [settings.approachSpeed, settings.audioOffsetMs]
  );
  ...
  const handleDrumHit = useCallback(
    (part: DrumPartId, hitTime: number) => {
      ...
      // Find the earliest unhit note for this part within judgment window (±140ms)
      const candidateNotes = notesRef.current.filter(
        (n) => n.part === part && !n.hit && !n.missed && Math.abs(n.time - currentAudioTime) <= 0.15
      );
  ...
  return (
    <div className="relative w-full h-full min-h-[580px] flex flex-col justify-between p-2 sm:p-4 bg-slate-950 select-none">
      {/* TOP HUD BAR */}
      <div className="w-full max-w-lg mx-auto flex flex-col gap-2 z-20">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="bg-slate-900/80 px-2.5 py-1.5 rounded-xl border border-slate-800/80 flex flex-col justify-center">
            <div className="flex justify-between text-[10px] text-slate-400 font-mono mb-1">
              <span>TIME</span>
              <span>
                {Math.floor(currentTime)}s / {song.duration}s
              </span>
            </div>
      ...
      {/* CENTER STAGE: TOP-DOWN DRUM SET & JUDGMENT DISPLAY */}
      <div className="relative flex-1 flex flex-col items-center justify-center my-1 sm:my-2">
        ...
        {/* Mascot Cheer Partner */}
        <div className="absolute top-2 left-2 z-30 pointer-events-none hidden sm:block">
          <MascotCharacter
            mascotId={settings.avatarId || 'pokota'}
            reaction={scoreState.isFever ? 'fever' : scoreState.combo > 10 ? 'combo' : 'idle'}
            combo={scoreState.combo}
            showSpeechBubble={scoreState.combo > 5 || scoreState.isFever}
          />
        </div>

        {/* Top-Down Drum Set Component */}
        <DrumSet
